...
export default class AdminAuthController {
    ...
    constructor(cfg: Configurations, adminAuthService: AdminAuthService, adminAuthMiddleware: AdminAuthMiddleware) {
        this.cfg = cfg;
    ...
    async googleCallback(req: FastifyRequest<{ Querystring: { code?: string, state?: string, error?: string } }>, res: FastifyReply) {
        ...
        try {
            const { accessToken, refreshToken, admin } = await this.adminAuthService.handleGoogleCallback(code, state);
            setAdminAuthCookies(res, { accessToken, refreshToken });
            this.logger.info({ message: "Admin login successful", data: { adminId: admin.id, email: admin.email } });
            res.redirect(this.cfg.admin.frontendOrigin);
        } catch (err) {
            if (err instanceof TError) {
                this.logger.warn({ message: err.message, error: err });
                return res.redirect(`${this.cfg.admin.frontendOrigin}/login?error=${encodeURIComponent(err.message)}`);
            }
