...
import { END_CREDIT_ROLL_CHANNEL } from "@/services/widget/endCredit/endCredit.service";
import { TError } from "@/errors";

export default class EndCreditEventController {
    ...
    async sse(req: FastifyRequest<{ Params: { userId: string }, Querystring: { key: string } }>, res: FastifyReply) {
        ...
        logger = this.logger.setContext("controller.endCreditEvent.sse", req.id);
        logger.info({ message: "SSE connection attempt", data: { userId } });

        try {
            const isValid = await this.widgetService.validateOverlayAccess(req.id, userId, key);
            if (!isValid) {
            ...
            await sub.subscribe(END_CREDIT_ROLL_CHANNEL, (message) => {
                ...
                if (payload.userId === userId) {
                    const { userId: _omitted, ...rollData } = payload;
                    res.sse({
                        event: "roll",
    ...
    public disconnectUser(userId: string) {
        ...
        const userConns = this.connections.get(userId);
        if (userConns) {
            logger = this.logger.setContext("controller.endCreditEvent.disconnectUser");
