...
export default class CanvasEventController {
    ...
    async sse(req: FastifyRequest<{ Params: { userId: string }, Querystring: { key: string } }>, res: FastifyReply) {
        ...
        this.logger.setContext("controller.canvasEvent.sse");
        this.logger.info({ message: "SSE connection attempt", data: { userId } });

        try {
            const isValid = await this.canvasService.validateOverlayAccess(userId, key);
            if (!isValid) {
            ...
            await sub.subscribe("canvas:play", (message) => {
    ...
    public disconnectUser(userId: string) {
        const userConns = this.connections.get(userId);
        if (userConns) {
            this.logger.setContext("controller.canvasEvent.disconnectUser");
