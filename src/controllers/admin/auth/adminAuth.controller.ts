...
export default class AdminAuthController {
  ...
  async googleCallback(
    ...
    res: FastifyReply
        ...
        let logger: TLogger = this.logger;
    logger = this.logger.setContext('controller.adminAuth.googleCallback', req.id);
    const { code, state, error } = req.query;

    if (error || !code || !state) {
      logger.warn({ message: 'Google callback missing code/state', data: { error } });
    ...
    try {
      ...
      if (err instanceof TError) {
        logger.warn({ message: err.message, error: err });
        return res.redirect(
          `${this.cfg.admin.frontendOrigin}/login?error=${encodeURIComponent(err.message)}`
        );
      }
      logger.error({ message: 'Google callback failed', error: err as Error });
  ...
  async refresh(req: FastifyRequest, res: FastifyReply) {
        let logger: TLogger = this.logger;
    logger = this.logger.setContext('controller.adminAuth.refresh', req.id);
    ...
    try {
      ...
      logger.error({ message: 'Admin token refresh failed', error: err as string | Error });
      clearAdminAuthCookies(res);
  ...
  async logout(req: FastifyRequest, res: FastifyReply) {
    ...
    try {
      ...
      logger.error({ message: 'Admin logout failed', error: err as Error });
      clearAdminAuthCookies(res);
      res.status(500).send({ message: 'Internal Server Error' });
    }
  }
  ...
  async me(req: FastifyRequest, res: FastifyReply) {
        let logger: TLogger = this.logger;
    logger = this.logger.setContext('controller.adminAuth.me', req.id);
