...
export default class RandomDBDKillerController {
    ...
    async get(req: FastifyRequest, res: FastifyReply) {
        ...
        logger = this.logger.setContext("controller.randomDBDKiller.get", req.id);
        ...
        try {
            ...
            logger.error({ message: "Failed to get random dbd killer config", data: { userId: user.id }, error: error as Error });
    ...
    async create(req: FastifyRequest, res: FastifyReply) {
        let logger: TLogger = this.logger;
        logger = this.logger.setContext("controller.randomDBDKiller.create", req.id);
        const user = getUserFromRequest(req);
        if (!user) {
            logger.warn({ message: "Unauthorized access attempt" });
            return res.status(401).send({ message: "Unauthorized" });
        ...
        try {
            ...
            logger.error({ message: "Failed to create random dbd killer config", data: { userId: user.id }, error: error as Error });
            res.status(500).send({ message: "Internal Server Error" });
        }
    }
    ...
    async update(req: FastifyRequest, res: FastifyReply) {
        let logger: TLogger = this.logger;
        logger = this.logger.setContext("controller.randomDBDKiller.update", req.id);
        const user = getUserFromRequest(req);
        if (!user) {
            logger.warn({ message: "Unauthorized access attempt" });
            return res.status(401).send({ message: "Unauthorized" });
        ...
        try {
            ...
            logger.error({ message: "Failed to update random dbd killer config", data: { userId: user.id }, error: error as Error });
            res.status(500).send({ message: "Internal Server Error" });
        }
    }
    ...
    async delete(req: FastifyRequest, res: FastifyReply) {
        let logger: TLogger = this.logger;
        logger = this.logger.setContext("controller.randomDBDKiller.delete", req.id);
        const user = getUserFromRequest(req);
        if (!user) {
            logger.warn({ message: "Unauthorized access attempt" });
            return res.status(401).send({ message: "Unauthorized" });
        ...
        try {
            ...
            logger.error({ message: "Failed to delete random dbd killer config", data: { userId: user.id }, error: error as Error });
            res.status(500).send({ message: "Internal Server Error" });
        }
    }
    ...
    async refreshKey(req: FastifyRequest, res: FastifyReply) {
        let logger: TLogger = this.logger;
        logger = this.logger.setContext("controller.randomDBDKiller.refreshKey", req.id);
        const user = getUserFromRequest(req);
        if (!user) {
            logger.warn({ message: "Unauthorized access attempt" });
            return res.status(401).send({ message: "Unauthorized" });
        ...
        try {
            ...
            logger.error({ message: "Failed to refresh random dbd killer overlay key", data: { userId: user.id }, error: error as Error });
            res.status(500).send({ message: "Internal Server Error" });
        }
    }
}
