...
describe("SpotifySongRequestService", () => {
    ...
    describe("create", () => {
        ...
        it("should create config and subscribe to EventSub when no existing subscription", async () => {
            ...
            expect(mockUserRepo.get).toHaveBeenCalledWith(transactionId, "user_1");
            expect(twitchAppAPI.eventSub.subscribeToChannelChatMessageEvents).toHaveBeenCalledWith("twitch_1", "transport");
            expect(mockSpotifyRepo.create).toHaveBeenCalledWith(transactionId, expect.objectContaining({
                twitch_id: "twitch_1",
                owner_id: "user_1",
                overlay_key: "mocked_hex",
            }));
            expect(mockWidgetService.setInitialEnabled).toHaveBeenCalledWith(transactionId, mockConfig.widget_id, "user_1");
    ...
    describe("getByUserId", () => {
        it("should return config when found", async () => {
            ...
            expect(mockSpotifyRepo.getByOwnerId).toHaveBeenCalledWith(transactionId, "user_1");
            expect(result).toBe(mockConfig);
        });
    ...
    describe("update", () => {
        it("should update config successfully when authorized", async () => {
            ...
            const result = await service.update(transactionId, "user_1", updateData);

            expect(mockSpotifyRepo.update).toHaveBeenCalledWith(transactionId, "ssr_1", updateData);
        ...
        it("should throw ForbiddenError if user does not own the config", async () => {
            ...
            await expect(service.update(transactionId, "user_1", {})).rejects.toThrow(ForbiddenError);
        });
    });
    ...
    describe("delete", () => {
        it("should delete config successfully when authorized", async () => {
            ...
            expect(mockSpotifyRepo.delete).toHaveBeenCalledWith(transactionId, "ssr_1");
        });

        it("should return early without error if config not found", async () => {
            mockSpotifyRepo.getByOwnerId.mockResolvedValue(null);

            await expect(service.delete(transactionId, "user_1")).resolves.toBeUndefined();
        ...
        it("should throw ForbiddenError if user does not own the config", async () => {
            ...
            await expect(service.delete(transactionId, "user_1")).rejects.toThrow(ForbiddenError);
        });
    });
    ...
    describe("getByTwitchId", () => {
        it("should return config from repository", async () => {
            ...
            const result = await service.getByTwitchId(transactionId, "twitch_1");

            expect(mockSpotifyRepo.getByTwitchId).toHaveBeenCalledWith(transactionId, "twitch_1");
            expect(result).toBe(mockConfig);
        });
    ...
    describe("test", () => {
        it("should call handleTwitchEvent with a fake event using the config's twitch ID and reward ID", async () => {
            ...
            await service.test(transactionId, "user_1");
        ...
        it("should throw error if twitch_reward_id is not configured", async () => {
            ...
            await expect(service.test(transactionId, "user_1")).rejects.toThrow("No reward configured");
        });
    });
    ...
    describe("insertSpotifyTrack", () => {
        it("should insert track by search query and return track info", async () => {
            ...
            const result = await service.insertSpotifyTrack(transactionId, "user_1", "my song artist a");

            expect(mockSpotify.createUserAPI).toHaveBeenCalledWith(transactionId, "user_1");
        ...
        it("should insert track by Spotify URL using tracks.get", async () => {
            ...
            const result = await service.insertSpotifyTrack(transactionId, "user_1", "https://open.spotify.com/track/abc123");
        ...
        it("should throw error when search returns no results", async () => {
            ...
            await expect(service.insertSpotifyTrack(transactionId, "user_1", "unknown song")).rejects.toThrow("Track not found");
        });
        ...
        it("should throw NoActiveDeviceError when Spotify returns NO_ACTIVE_DEVICE", async () => {
            ...
            await expect(service.insertSpotifyTrack(transactionId, "user_1", "some song")).rejects.toThrow(NoActiveDeviceError);
        });
        ...
        it("should silently ignore Unexpected token errors from addItemToPlaybackQueue", async () => {
            ...
            const result = await service.insertSpotifyTrack(transactionId, "user_1", "some song");
    ...
    describe("handleTwitchEvent", () => {
        ...
        it("should return early if channel_points_custom_reward_id is absent", async () => {
            await service.handleTwitchEvent(transactionId, { ...mockEvent, channel_points_custom_reward_id: null });
        ...
        it("should return early if config not found for broadcaster", async () => {
            ...
            await service.handleTwitchEvent(transactionId, mockEvent);
        ...
        it("should return early if reward ID does not match config", async () => {
            await service.handleTwitchEvent(transactionId, { ...mockEvent, channel_points_custom_reward_id: "wrong_reward" });
        ...
        it("should not set replyParentMessageId for test mode messages", async () => {
            ...
            await service.handleTwitchEvent(transactionId, {
                ...mockEvent,
