...
describe("WidgetService", () => {
    ...
    describe("authorizeOwnership", () => {
        it("should not throw if owner matches", async () => {
            ...
            await expect(service.authorizeOwnership(transactionId, "u_1", "w_1")).resolves.not.toThrow();
        });

        it("should throw ForbiddenError if owner mismatches", async () => {
            mockWidgetRepo.get.mockResolvedValue({ id: "w_1", owner_id: "u_2" } as any);
            await expect(service.authorizeOwnership(transactionId, "u_1", "w_1")).rejects.toThrow(ForbiddenError);
        });
    });

    describe("authorizeTierUsage", () => {
        it("should allow disabling always", async () => {
            await expect(service.authorizeTierUsage(transactionId, "u_1", "w_1", false)).resolves.not.toThrow();
        });
        ...
        it("should allow enabling for Pro tier without limit", async () => {
            ...
            await expect(service.authorizeTierUsage(transactionId, "u_1", undefined, true)).resolves.not.toThrow();
        });
        ...
        it("should throw WidgetQuotaLimitError for Free tier if limit reached (new widget)", async () => {
            ...
            await expect(service.authorizeTierUsage(transactionId, "u_1", undefined, true)).rejects.toThrow(WidgetQuotaLimitError);
        });
        ...
        it("should throw WidgetQuotaLimitError for Free tier if limit reached (existing widget)", async () => {
            ...
            await expect(service.authorizeTierUsage(transactionId, "u_1", "w_1", true)).rejects.toThrow(WidgetQuotaLimitError);
        });

        it("should throw NotFoundError if widget missing during tier check", async () => {
            mockUserService.get.mockResolvedValue({ tier: UserTier.FREE_TIER, extra_widget_quota: 0 } as any);
            mockWidgetRepo.get.mockResolvedValue(null);
            await expect(service.authorizeTierUsage(transactionId, "u_1", "w_1", true)).rejects.toThrow(NotFoundError);
        });

        it("should handle error during tier authorization", async () => {
            mockUserService.get.mockRejectedValue(new Error("API Error"));
            await expect(service.authorizeTierUsage(transactionId, "u_1")).rejects.toThrow("API Error");
        });
    });
    ...
    describe("update", () => {
        it("should update successfully", async () => {
            ...
            await service.update(transactionId, "w_1", "u_1", { enabled: true });
        ...
        it("should throw NotFoundError if missing", async () => {
            mockWidgetRepo.get.mockResolvedValue(null);
            await expect(service.update(transactionId, "w_1", "u_1", {})).rejects.toThrow(NotFoundError);
        });
    });
    ...
    describe("updateEnable", () => {
        it("should update enable status successfully", async () => {
            ...
            await service.updateEnable(transactionId, "w_1", "u_1", true);

            expect(mockWidgetRepo.update).toHaveBeenCalledWith(transactionId, "w_1", { enabled: true });
        });
        ...
        it("should throw WidgetQuotaLimitError if authorizeTierUsage throws WidgetQuotaLimitError", async () => {
            ...
            await expect(service.updateEnable(transactionId, "w_1", "u_1", true)).rejects.toThrow(WidgetQuotaLimitError);
        });
        ...
        it("should rethrow other errors from authorizeTierUsage", async () => {
            ...
            await expect(service.updateEnable(transactionId, "w_1", "u_1", true)).rejects.toThrow("Generic error");
        });
    });
    ...
    describe("setInitialEnabled", () => {
        it("should enable if free tier and no active widgets", async () => {
            ...
            await service.setInitialEnabled(transactionId, "w_1", "u_1");

            expect(mockWidgetRepo.update).toHaveBeenCalledWith(transactionId, "w_1", expect.objectContaining({ enabled: true }));
        });
        ...
        it("should disable if free tier and already 1 active widget", async () => {
            ...
            expect(mockWidgetRepo.update).toHaveBeenCalledWith(transactionId, "w_1", expect.objectContaining({ enabled: false }));
        });
    });
    ...
    describe("delete", () => {
        it("should delete successfully", async () => {
            mockWidgetRepo.get.mockResolvedValue({ id: "w_1", owner_id: "u_1" } as any);
            await service.delete(transactionId, "w_1", "u_1");
            expect(mockWidgetRepo.delete).toHaveBeenCalledWith(transactionId, "w_1");
        });

        it("should throw NotFoundError if missing", async () => {
            mockWidgetRepo.get.mockResolvedValue(null);
            await expect(service.delete(transactionId, "w_1", "u_1")).rejects.toThrow(NotFoundError);
        });
    });

    describe("updateOverlayKey", () => {
        it("should update key", async () => {
            await service.updateOverlayKey(transactionId, "w_1", "new_key");
            expect(mockWidgetRepo.updateOverlayKey).toHaveBeenCalledWith(transactionId, "w_1", "new_key");
        });
    });
    ...
    describe("validateOverlayAccess", () => {
        it("should return true if owner matches and key matches", async () => {
            ...
            const result = await service.validateOverlayAccess(transactionId, "u_1", "key_1");
    ...
    describe("get", () => {
        it("should use cache", async () => {
            ...
            const result = await service.get(transactionId, "w_1");
        ...
        it("should throw NotFoundError if missing", async () => {
            (redis.get as jest.Mock).mockResolvedValue(null);
            mockWidgetRepo.get.mockResolvedValue(null);
            await expect(service.get(transactionId, "w_1")).rejects.toThrow(NotFoundError);
        });
    });
    ...
    describe("list", () => {
        it("should return list of widgets", async () => {
            ...
            const result = await service.list(transactionId, "u_1", { page: 1, limit: 10 });
    ...
    describe("refreshOverlayKey", () => {
        it("should refresh key", async () => {
            await service.refreshOverlayKey(transactionId, "w_1");
            expect(mockWidgetRepo.updateOverlayKey).toHaveBeenCalledWith(transactionId, "w_1", "mocked_uuid");
        });
    });
    ...
    describe("getFirstEnabled", () => {
        it("should return first enabled widget", async () => {
            ...
            const result = await service.getFirstEnabled(transactionId, "u_1");
            expect(result.id).toBe("w_1");
        });

        it("should throw NotFoundError if none found", async () => {
            mockWidgetRepo.getFirstEnabled.mockResolvedValue(null);
            await expect(service.getFirstEnabled(transactionId, "u_1")).rejects.toThrow(NotFoundError);
        });
    });
});
