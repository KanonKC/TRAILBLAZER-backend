...
export default class TwitchChannelFollowEvent {
    ...
    async handle(req: FastifyRequest, res: FastifyReply) {
        ...
        logger = this.logger.setContext("event.twitch.channelFollow.handle", req.id);
        ...
        if (body.subscription.status === "enabled") {
            logger.info({ message: "Handling channel follow event", data: event })
            try {
                await this.endCreditService.handleTwitchChannelFollowEvent(event, req.id)
            } catch (err: any) {
